...
- **Retraining**: The model was trained on 2021 data. It should be retrained annually as new AIS data becomes available, and monitored for drift — particularly after rapid economic changes (e.g., post-COVID funding changes, inflation impacts from conflict such as ones involving the Middle East oil suppliers). The Excel file for other years has a different file format in terms of column headings and staffing levels as a range, not a number. When trying to run the code for a different year, I ran into these issues so standardising other years to have the same format was also attempted but not satisfactorily achieved.
